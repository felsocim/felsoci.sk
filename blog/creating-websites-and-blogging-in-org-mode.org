...
At the end, I include the common set-up Org file (see Section [[#setup-file]]).
...
Next, I override the default LaTeX publishing command to use =latexmk= in order
to ensure the bibliography entries, if any, are published correctly.
...
Finally, I define a utility function allowing me to change the output directory
...
be published into the ~public~ directory located in the root of the project.
