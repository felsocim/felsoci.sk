...
available for download from the home page. There is also my personal
bibliography file in ~cv~. ~images~ naturally contains all the image files
featured on the site. ~public~ can be used to store the exported website (see
more in [[#general-configuration][General configuration]]). The custom CSS
sheets reside in ~styles~ where ~htmlize.css~ stylizes syntax highlighting in
source code blocks and ~custom.css~ defines the look and feel of all the other
elements of the website. ~shared~ holds the common static header and footer
files as well as an Org setup file providing some common document configuration,
e.g. for bibliography. The Org documents corresponding to the content pages are
stored in the root of the project's folder as well as the global LaTeX
bibliography file ~references.bib~ for the entire website. Finally, the Emacs
